...
public class CraftingHandler extends RestrictionHandler {
    ...
    private void onItemCrafted(PrepareItemCraftEvent event) {
        ...
        if (!event.getViewers().isEmpty()) {
            ...
            Optional<Restriction> bannedInfo = itemKey.isRestricted(p, RestrictionTypes.CRAFTING, p.getLocation());
            ...
            Optional<Restriction> bannedInfo = itemKey.isRestricted(null, RestrictionTypes.CRAFTING, null);
            if (bannedInfo.isPresent()) {
                event.getInventory().setResult(null);
            }
        }
